...
Suppose your homestead sensor system has `.env` containing:

```
DATABASE_URL=postgres://user:password@host
JWT_SECRET=super-secret-key
MQTT_PASSWORD=real-device-password
WIFI_SSID=MyNetwork
WIFI_PASSWORD=MyPassword123
ESP32_API_KEY=abc123xyz789
```

This file must never be committed. Instead: ignore `.env`, commit `.env.example` with placeholder values:
...
Build output is generated from source code. Examples: `dist/`, `build/`, `coverage/`, `*.min.js`, compiled binaries, transpiled files. Generated files can be recreated, pollute history, change constantly, and are large. Git should track source, not artifacts.
...
Your homestead automation repo might have:
- `.env` with real credentials → ignore
- `node_modules/` from npm installs → ignore
- `__pycache__/` from Python → ignore
- `logs/` directory with runtime logs → ignore
- `.DS_Store` from macOS → ignore
- `dist/` or `build/` if you compile anything → ignore
- `*.db` SQLite databases → ignore (unless they're seed data)

You commit: source code, config templates, dependency manifests (`package.json`, `requirements.txt`), documentation.
...
`*` matches any characters. Example: `*.log` matches `error.log`, `server.log`, `access.log`.

### Double Star (`**`)

`**` matches directories recursively. Example: `**/temp/` matches `temp/`, `src/temp/`, `data/archive/temp/`.
...
Leading slash means: only match at repository root. Example: `/.env` ignores only `<repo-root>/.env`, not `config/.env`.

### Negation

You can un-ignore something using `!`. Example:
...
Your homestead repo might need:
...
Explanation:
- `__pycache__/` and `*.pyc` → compiled bytecode
...
If you use SQLite: `database.db`. You must decide: Is the database source data? Or is it generated runtime state? For most apps: `*.db` because production DB should not be committed, and local dev DB is disposable.
...
For a homestead automation system with Node.js API, Python scripts, and ESP32 code:
...
You can define a global ignore file:
...
Set up global ignore for OS files:
...
If a file is already tracked, adding it to `.gitignore` does not remove it. You must:

```bash
git rm --cached .env
```

Then commit. After that, `.gitignore` prevents re-tracking. This is a common mistake.

### Homestead Example: Removing Tracked Secrets

You accidentally committed `.env` with real credentials. Remove it from tracking:
...
You committed `logs/server.log` before setting up `.gitignore`. Remove it:
...
When you start, you might have:
- A few Python scripts
- One `.env` file
- No build output

As the project grows:
- Node.js API with `node_modules/`
- Python scripts with `__pycache__/`
- Build processes creating `dist/`
- Multiple `.env` files (dev, staging, prod)
- Log files accumulating
- SQLite databases for testing

Without `.gitignore`, every commit would include:
...
**Fix:** Remove from tracking (`git rm --cached .env`), add to `.gitignore`, commit. Rotate the exposed credentials immediately.
...
**Fix:** Use negation: `data/*` then `!data/example.json` and `!data/README.md`.
...
Your homestead API uses Node.js, but you have Python scripts for data processing:
...
Your ESP32 code uses MicroPython:
...
Your homestead has multiple components:
- Node.js API server
...
## 10) Review: .gitignore in One Page

Before moving on, you should be able to state in your own words:

1. **What should never be committed?** Secrets, generated files, dependencies, logs, OS files, machine-specific config.

2. **How do you write .gitignore patterns?** One pattern per line. Use `*` for wildcards, `**` for recursive directories, `/` for root-only or directory-only, `!` for negation.

3. **What's the difference between ignoring and untracking?** `.gitignore` only affects untracked files. To stop tracking an already-tracked file, use `git rm --cached`.

4. **What's the pattern for secrets?** Ignore real files (`.env`, `.env.local`), commit templates (`.env.example`) with placeholders.

5. **How do you handle multiple languages?** Include patterns for each: Node.js (`node_modules/`), Python (`__pycache__/`, `venv/`), etc.

If any of these is fuzzy, revisit that section. The next chapter assumes you understand how to exclude files from version control.
...
- `.gitignore` only affects untracked files; use `git rm --cached` to stop tracking existing files.
